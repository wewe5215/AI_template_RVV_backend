...
class ConvDepthwiseBiasReluTestCase(unittest.TestCase):
    ...
    def test_fp32(self, batch=1):
        ...
        W = Tensor(shape=[32, 3, 3, 1], dtype="float32", name="input_1", is_input=True)
        B = Tensor(shape=[32], dtype="float32", name="input_2", is_input=True)
        OP = ops.conv2d_depthwise_bias_relu(stride=1, pad=1, dilate=1, group=groups)
        Y = OP(X, W, B)
        Y._attrs["name"] = "output_0"
        Y._attrs["is_output"] = True
        module = compile_model(Y, target, "./tmp", "conv2d_dw_bias")

        X_pt = torch.randn(batch, 32, *size)
        W_pt = torch.randn(32, 1, 3, 3)
        B_pt = torch.randn(32)
        Y_pt = torch.nn.functional.conv2d(
            X_pt, W_pt, bias=B_pt, padding=1, groups=groups
        )
