...
def load_data(batch_size, model_name):
    ...
    inputs = {}
    inputs["qkv"] = FakeTorchTensor(data["qkv"])
    inputs["cu_seqlens"] = FakeTorchTensor(data["cu_seqlens"])
    y = FakeTorchTensor(data["y"])

    return inputs, y

def run(model_name, batch_size, mod=None, graph_mode=True):
    inputs, y = load_data(batch_size, model_name)
    mod = Model(os.path.join(f"./{model_name}", "test.so"))
    mod.run_with_tensors(inputs, [y])

    output_file = f"output_file_{model_name}_{batch_size}.npz"
    y_output_np = y.cpu().detach().numpy().astype(np.float32)
