...
@registry.reg("rvv.cpu_lib")
def cpu_lib(src_prefix, dst_prefix=None):
    if dst_prefix is None:
        dst_prefix = tempfile.mkdtemp()
    lib_dst = os.path.join(dst_prefix, "cpu_lib")
...
@registry.reg("rvv.gen_xnnpack_ops")
def gen_ops(arch):
    import cpu_lib

    args = Args(arch)
    manifest = cpu_lib.manifest.Manifest(args)
    try:
        func = getattr(cpu_lib.generator, "Generate" + arch.upper())
        func(manifest, args.rvv_version)
    except AttributeError as exc:
        raise NotImplementedError(
            "Arch " + arch + " is not supported by current cpu lib."
        ) from exc
