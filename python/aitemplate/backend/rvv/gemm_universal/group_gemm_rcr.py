...
from aitemplate.backend.rvv.gemm_universal import common, group_common
...
@registry.reg("rvv.group_gemm_rcr.config")
def group_rcr_config(func_attrs):
    common.make_fproc(func_attrs, RCR)


@registry.reg("rvv.group_gemm_rcr.gen_profiler")
...
@registry.reg("rvv.group_gemm_rcr.gen_function")
def gen_function(
    func_attrs,
    exec_cond_template,
    shape_eval_template,
...
@registry.reg("rvv.group_gemm_rcr.func_decl")
...
@registry.reg("rvv.group_gemm_rcr.func_call")
def gen_function_call(func_attrs, indent="  "):
    ndims = 2
    return group_common.gen_function_call(func_attrs, ndims)


@registry.reg("rvv.group_gemm_rcr.filter")
