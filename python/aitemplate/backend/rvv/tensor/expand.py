...
from aitemplate.backend.rvv.tensor import expand_static_shape  # noqa: F401


@registry.reg("rvv.expand.func_decl")
def gen_function_decl(func_attrs: Dict[str, Any]) -> str:
    if func_attrs["optimize_fixed_dims"] and func_attrs["non_head_dims_are_fixed"]:
        func = registry.get("rvv.expand.static.func_decl")
        return func(func_attrs)
    x = func_attrs["inputs"][0]
    func_name = func_attrs["name"]
    cuda_spec: RVVSpec = RVVSpec()
...
@registry.reg("rvv.expand.gen_function")
def gen_function(func_attrs: Dict[str, Any]) -> str:
    if not (
        ...
        func = registry.get("rvv.expand.static.gen_function")
        return func(func_attrs)


@registry.reg("rvv.expand.func_call")
def gen_function_call(func_attrs: Dict[str, Any], indent: str = "  ") -> str:
    if not (
        ...
        func = registry.get("rvv.expand.static.func_call")
