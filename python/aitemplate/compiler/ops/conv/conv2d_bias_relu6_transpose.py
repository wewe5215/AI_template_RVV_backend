...
#
"""
Fused conv2d_bias_relu6_transpose op.
"""
from aitemplate.compiler.ops.conv.common_conv2d_bias_relu6_activation import (
    conv2d_bias_relu6_activation,
)
...
class conv2d_bias_relu6_transpose(conv2d):
    r"""Conv2d with bias + relu6 + transpose.

    Applies a 2D convolution on input in shape (N, H, W, C_in), adds a bias in shape (C_out), performs relu6 and produces output in shape (N, H_out, W_out, C_out). N is batch size, H, W are the height and width of the input images in pixels, and C is the number of channels.
    ...
    .. code-block:: python
        ...
        OP = aitemplate.compiler.ops.conv2d_bias_relu6_transpose(stride=1, pad=1, dilate=1)
        Result_ait = OP(X, W, B)
    ...
    def __init__(self, stride, pad, dilate=1, group=1) -> None:
        ...
        self._attrs["op"] = "conv2d_bias_relu6_transpose"
