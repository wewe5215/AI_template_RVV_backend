...
class MobileNetV2Verification(unittest.TestCase):
    def test_mobilenetv2(self):
        ...
        with torch.no_grad():
            y_pt = pt_model(x_pt)
        weights_file = f"mobilenetv2_weights_file.npz"
        io_file = f"mobilenetv2_io_tensors_{batch_size}.npz"
        np_weights = {}
        ...
        np.savez(f"mobilenetv2_{batch_size}_y_pt.npz", y=y_pt.cpu().numpy())
