...
batch_size = 1 # profile result could varying with batch size
pruning_ratio = 0.75
resnet_depth = 50  # Change this to 18, 34, 50, 101, or 152
result_file = f"results_prune_{int(pruning_ratio*100)}_resnet{resnet_depth}.txt"
profile_summary_model_name = f"cnhw_pruned_{int(pruning_ratio*100)}_resnet{resnet_depth}_{batch_size}"
...
def perform_pruning(model, pruning_ratio=0.5, depth=50):
    """
    ...
    for layer_name, module in model.named_modules():
        ...
        weight_tensor = module.weight.detach().cpu().numpy().astype(np.float32)
...
def train(args, accelerator, train_data, eval_data, model, is_regression=False, depth=50):
    ...
    scheduler_step_size = 30 * num_update_steps_per_epoch  # steps corresponding to 5 epochs.
    ...
    for layer_name, module in model.named_modules():
        if not isinstance(module, torch.nn.Conv2d):
            continue
        full_name = layer_name + ".weight"
        print(full_name)

        if 'layer1' in full_name or ('layer2.0' in full_name and ('conv1' in full_name or 'downsample' in full_name)):
...
if __name__ == "__main__":
    ...
    # Map depth to model function and weights
    resnet_models = {
    ...
    # Check if the requested depth is valid
    if resnet_depth not in resnet_models:
        raise ValueError(f"Unsupported ResNet depth {resnet_depth}. Supported depths: {list(resnet_models.keys())}")

    # Load model and weights
    model_fn, weight_enum = resnet_models[resnet_depth]
    weights = weight_enum
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.to(device)
    perform_pruning(model, pruning_ratio=pruning_ratio, depth=resnet_depth)
    # Train the model.
    train(args, accelerator, train_data, eval_data, model, is_regression=False, depth=resnet_depth)
