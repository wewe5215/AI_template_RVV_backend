...
class BasicBlock(CNNBlockBase):
    ...
    def __init__(self, in_channels, out_channels, *, block_idx, stride=1, norm="BN", activation="ReLU"):
        ...
        if in_channels != out_channels or stride != 1:
            self.downsample_0 = nn.Conv2dCNHWPruningBias(in_channels, out_channels, kernel_size=1, stride=stride, padding=0, dtype="float")
        else:
            self.downsample_0 = None

        self.conv1 = nn.Conv2dCNHWPruningBiasRelu(in_channels, out_channels, kernel_size=3, stride=stride, padding=1)
        self.conv2 = nn.Conv2dCNHWPruningBiasAddRelu(out_channels, out_channels, kernel_size=3, stride=1, padding=1)
        ...
        if block_idx < 1:
            ...
            if activation == "ReLU":
                if stride_3x3 == 2:
                    ...
                    conv_op1 = nn.Conv2dCNHWPruningBiasRelu
                    conv_op2 = nn.Conv2dCNHWPruningBiasRelu
                conv_op_add = nn.Conv2dCNHWPruningBiasAddRelu
            else:
                raise NotImplementedError
        else:
            if activation == "ReLU":
